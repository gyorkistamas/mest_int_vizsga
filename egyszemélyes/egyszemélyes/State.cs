...
namespace egyszemélyes
{
    ...
    {
        public const int TABLE_ROW_COUNT = 3;
        public const int TABLE_COL_COUNT = 3;
        ...
        public State()
        {
            Table = new int[TABLE_ROW_COUNT, TABLE_COL_COUNT]
            {
                { 01, 08, 05 },
                { 07, 09, 04 },
                { 06, 03, 02 },
        ...
        {
            if (table.GetLength(0) != TABLE_ROW_COUNT ||
                table.GetLength(1) != TABLE_COL_COUNT) 
            {
                throw new Exception($"A {TABLE_ROW_COUNT}x{TABLE_COL_COUNT} matrix is required!");
            }
        ...
        public override string ToString()
        {
            ...
            for (int i = 0; i < this.Table.GetLength(0); i++)
            {
                ...
                sr.Append('\n');
                for (int j = 0; j < this.Table.GetLength(1) * 4 + this.Table.GetLength(1) + 1; j++)
                {
                    sr.Append("-");
                }
                sr.Append('\n');
